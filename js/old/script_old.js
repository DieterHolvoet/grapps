...
appshopper.handler = function(data, platform) {
    ...
    data = data.replace(/src/gi,'source');
    $data = $(data);
    $data.find(".appdetails > .pricedrop").each(function(i, eventItem) {
        var $eventItem = $(eventItem);
        appshopper.apps.push(new App());
        var loc = appshopper.apps.length - 1;
        appshopper.apps[loc].platform = platform;
        appshopper.apps[loc].title = $eventItem.find("h3 a").text();
        appshopper.apps[loc].icon = $eventItem.find("a img").attr("source");
        appshopper.apps[loc].url = "http://appshopper.com" + $eventItem.find("a").attr("href");

        if($eventItem.find(".pricedrop sup").text() === "") {
            appshopper.apps[loc].op = $eventItem.find(".pricedrop span").text();
        } else {
            appshopper.apps[loc].op = $eventItem.find(".pricedrop").contents().filter(function() {
                return this.nodeType === 3;
            }).text() + '.' + $eventItem.find(".pricedrop sup").text();
        }

        if($eventItem.find("dl dd:nth-of-type(2)").text() !== "No data") {
           appshopper.apps[loc].rating = parseRating($eventItem.find("dl dd:nth-of-type(2)").text().substr(0,2));
        }
    });
